...
## 2025-12-03
- **작업 내용**: 법령예고 크롤러 데이터 미표시 문제 해결
- **상세**:
  - 사용자로부터 입법예고 게시물이 업데이트되지 않는다는 제보 접수.
  - debug_moleg.py 및 브라우저 확인 결과, 크롤러는 정상적으로 최신 데이터를 수집하고 있음을 확인 (12/3 기준 12건 신규).
...
### UX: G2B Total Count & Mobile UI Refinement
**일시**: 2025-12-08

**작업 내용**:
1. **총 게시물 수 표시 개선**:
   - 기존: 1000건 제한된 개수만 표시되어 사용자 혼란.
   - 변경: API가 필터링(가격)이 없는 경우 DB 전체 개수(약 1.8만 건)를 반환하도록 수정. 가격 필터 적용 시에만 필터링된 개수 표시.
2. **모바일 카드 UI 개선 (2차)**:
   - 사용자 요청: 공고일과 마감일을 같이 보고 싶음.
   - 변경: 우측 하단에 '등록(공고)일'과 '마감일'을 상하로 배치하여 가독성 개선.
     - 상단: 등록/공고일 (파란색 강조)
     - 하단: 마감일 (만료 시 빨간색)
...
## 2025-12-10: 업체 분석 기능 강화 및 내보내기 기능 구현

### 1. 분담이행 계약 처리
**작업 내용**:
- 공동도급 방식이 '분담이행'인 계약은 금액 산정에서 자동 제외
- 계약명 앞에 빨간색 `[분담이행]` 배지 표시
- joint_type 필드를 API에서 조회하여 프론트엔드에 전달

**수정 파일**:
- `dashboard/app/api/company-stats/route.ts` (joint_type, is_joint_contract 필드 추가)
- `dashboard/app/company-analysis/page.tsx` (분담이행 배지 및 자동 제외 로직)

### 2. 공동도급 파트너 목록 표시
**작업 내용**:
- 공동도급 계약의 경우 용역명 아래에 파트너 업체명과 지분율 표시
- 형식: `• 업체명 (지분율%)`
- contract_partners 테이블에서 파트너 목록 조회

### 3. 변경 계약 표시
**작업 내용**:
- 착수일과 계약일의 연도가 다른 계약은 `[변경]` 배지 표시
- 착수일 기준으로 연도 분류 (수주 분석 시)

### 4. 분석 결과 내보내기 기능
**작업 내용**:
- **엑셀 내보내기**: xlsx 라이브러리 사용, 업체별 시트 생성
  - 포함 정보: 연도, 계약일, 계약명, 발주기관, 계약금액, 지분율, 수주금액, 분담이행 여부, 제외 여부
- **PNG 내보내기**: html2canvas 라이브러리 사용, 4K 고해상도 (scale: 4)
  - 내보내기 전 모든 연도 섹션 자동 펼침
  - 결과 영역을 ref로 지정하여 해당 영역만 캡처
...
**수정 파일**:
- `dashboard/app/company-analysis/page.tsx` (내보내기 함수 및 버튼 UI)

### 5. UI 개선
- 결과 영역 상단에 "엑셀 다운로드", "PNG 다운로드 (4K)" 버튼 추가
- 내보내기 중 버튼 비활성화 및 로딩 표시

### 6. 제외 키워드 필터 기능 추가
**작업 내용**:
- 계약명에서 특정 키워드를 **제외**하여 검색할 수 있는 기능 추가
- 포함 키워드와 별도로 빨간색 테마의 제외 키워드 입력 필드 추가
- 제외된 키워드는 🚫 아이콘과 함께 빨간색 배지로 표시

**수정 파일**:
- `dashboard/app/company-analysis/page.tsx` (제외 키워드 상태, 함수, UI)
- `dashboard/app/api/company-stats/route.ts` (excludeKeywords 파라미터 파싱, SQL NOT ILIKE 조건)

**사용 예**:
- 포함: "영향평가" → 영향평가가 포함된 계약만 표시
- 제외: "착수", "설계변경" → 착수, 설계변경이 포함된 계약은 제외

---

## 2025-12-12: 혼합 방식 계약 처리 구현

### 배경
공동도급 계약은 3가지 방식이 존재:
- **공동이행**: 모든 업체가 비율대로 참여 (예: 70%, 30%)
- **분담이행**: 각 업체가 특정 부분을 100% 담당 → 금액 산정 제외 (기존 구현)
- **혼합방식**: 공동이행 + 분담이행 혼재 → **신규 구현**

### 혼합 방식 처리 규칙
- 같은 계약에 100%와 100% 미만 지분율이 동시에 존재하면 혼합으로 판별
- 혼합 계약에서 100% 지분율 업체 → 분담이행으로 간주, 금액 산정 **제외**
- 100% 미만 지분율 업체 → 정상적으로 `지분율 × 계약금액` 산정

### 수정 내용

**백엔드 API** (`dashboard/app/api/company-stats/route.ts`):
- YearlyData 인터페이스에 `is_mixed_contract` 필드 추가
- 파트너 조회 후 혼합 계약 여부 판별 로직 추가
- 100%와 100% 미만 동시 존재 시 `is_mixed_contract = true`

**프론트엔드 UI** (`dashboard/app/company-analysis/page.tsx`):
- ContractDetail 인터페이스에 `is_mixed_contract` 필드 추가
- `[혼합]` 보라색 배지 추가 (혼합 계약 표시)
- 파트너 목록에서 100% 지분율에 빨간색 "분담" 표기
- `getAdjustedStats` 및 `getAdjustedYearData` 함수에서 혼합 계약의 100% 지분율 자동 제외
