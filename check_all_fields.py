...
def check_all_varchar_fields():
    """모든 VARCHAR 필드의 최대 길이 확인"""
    print("=== 모든 VARCHAR 필드 길이 분석 ===\n")
    ...
    if issues:
        ...
        for field_name, max_length, actual_max, over_count in issues:
            ...
            for contract_no, value, length in samples:
                print(f"    • {contract_no}: {length}자 - {value[:80]}{'...' if len(str(value)) > 80 else ''}")
    else:
