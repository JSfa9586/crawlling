# 해양수산부_해역이용협의정보 서비스 API 개발 가이드

## 목차
1. 개요
2. 인증 및 공통 사항
3. API 분류별 상세
    3.1. 해역이용협의정보 목록 조회 (getOceansUseInfo2)
4. 에러 코드 및 트러블슈팅
5. 개발 체크리스트

## 1. 개요

본 문서는 **해양수산부_해역이용협의정보 서비스** API를 활용하여 개발을 진행하는 개발자를 위한 가이드라인입니다. 이 API는 해역이용협의 정보를 조회하는 기능을 제공합니다.

*   **API 서비스명:** 해양수산부_해역이용협의정보 서비스
*   **설명:** 접수연도, 사업명을 이용하여 사업번호, 접수일, 결과명, 진행단계명, 사업명, 사업시행자, 주소, 사업 시작/종료일, 사업비, 사업규모, 사업목적, 처분기관, 협의 상태, 위도, 경도 등 상세정보를 조회하는 서비스입니다.
*   **API 유형:** REST
*   **데이터 포맷:** JSON, XML
*   **키워드:** 해역, 해역이용, 협의정보
...
해역이용협의 정보를 목록 형태로 조회합니다.

*   **분류/카테고리:** 해역이용협의정보 조회
*   **기능 설명:** 접수연도, 사업명 등의 검색 조건을 이용하여 해역이용협의 정보를 조회합니다.
*   **요청 주소 (Endpoint URL):** `http://apis.data.go.kr/1192000/service/OceansUseService2/getOceansUseInfo2`
*   **서비스 URL:** `http://apis.data.go.kr/1192000/service/OceansUseService2`

#### 3.1.1. 요청 파라미터 상세 (Request Parameters)

| 항목명(국문) | 항목명(영문) | 필수/선택 | 타입 | 항목크기 | 샘플데이터 | 항목설명 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 서비스키 | ServiceKey | **필수** | String | 4 | 인증키 (URL-Encode) | 공공데이터포털에서 받은 인증키 |
...
| SEQUENCE | seq | 선택 | Integer | 10 | 6000 | SEQUENCE |
| 사업번호 | bizNum | 선택 | String | 30 | 1111441063 | 사업번호 |
| 접수연도 | acpYear | 필수 | String | 4 | 2013 | 접수연도 |
| 접수일 | acpDat | 선택 | String | 30 | 2013-04-01 | 접수일 |
| 결과명 | rstNm | 선택 | String | 20 | 조건부동의 | 결과명 |
| 진행단계명 | stpNm | 선택 | String | 20 | 협의완료 | 진행단계명 |
| 사업명 | bizNam | 선택 | String | 30 | 선박 피해방지용 침추 설치 (현대미포조선 3안벽 전면) | 사업명 |
| 사업시행자 | bizActNam | 선택 | String | 30 | ㈜현대미포조선 대표이사 최원길 | 사업시행자 |
| 사업유형명 | bizTypNm | 선택 | String | 30 | 공유수면점용 | 사업유형명 |
| 사업유형 기타 | bizTypEtc | 선택 | String | 1000 | 공유수면 점사용 | 사업유형 기타 |
| 주소시도 | adrSido | **필수** | String | 30 | 울산 | 주소시도 |
| 주소구군 | adrGugun | 선택 | String | 30 | 동구 | 주소구군 |
| 주소읍면동 | adrDong | 선택 | String | 30 | 방어동 | 주소읍면동 |
| 사업 소재지 상세주소 | adrDtl | 선택 | String | 100 | 1425번지 및 1381번지 공유수면 | 사업 소재지 상세주소 |
| 사업 시작일 | bldDat | 선택 | String | 30 | 2013-06-01 | 사업 시작일 |
| 사업 종료일 | endDat | 선택 | String | 30 | 2014-05-31 | 사업 종료일 |
| 사업비 | cst | 선택 | String | 12 | 사업비(원) |
| 사업규모 | bizSiz | 선택 | String | 1000 | 1,262.25㎡ (직접 12.25㎡ / 간접 1,250㎡) | 사업규모 |
| 사업목적 | bizAim | 선택 | String | 1000 | 선박 피해방지용 침추 설치 (현대미포조선 3안벽 전면) | 사업목적 |
| 처분기관 | prcIst | 선택 | String | 30 | 울산지방해양수산청 | 처분기관 |
| 비고 | etcDes | 선택 | String | 1000 | 비고 |
| 협의 상태 | dsSts | 선택 | String | 30 | 조건부동의 | 협의 상태 |
| 위도 | lat | 선택 | Float | 16 | 129.399 | 위도 |
| 경도 | lon | 선택 | Float | 16 | 35.503 | 경도 |
...
# ServiceKey, pageNo, numOfRows, ACP_YEAR는 필수 파라미터입니다.
...
curl -X GET "http://apis.data.go.kr/1192000/service/OceansUseService2/getOceansUseInfo2?ServiceKey={YOUR_SERVICE_KEY_URL_ENCODED}&pageNo=1&numOfRows=10&ACP_YEAR=2013&BIZ_NAM=%EC%84%A0%EB%B0%95%20%ED%94%BC%ED%95%B4%EB%B0%A9%EC%A7%80%EC%9A%A9%20%EC%B9%A8%EC%B6%94%20%EC%84%A4%EC%B9%98%20%28%ED%98%84%EB%8C%80%EB%AF%B8%ED%8F%AC%EC%A1%B0%EC%84%A0%203%EC%95%88%EB%B2%BD%20%EC%A0%84%EB%A9%B4%29&resultType=json"
```
      ...
      "items": [
        {
          ...
          "rstNm": "조건부동의",
          "stpNm": "협의완료",
          "bizNam": "선박 피해방지용 침추 설치 (현대미포조선 3안벽 전면)",
          "bizActNam": "㈜현대미포조선 대표이사 최원길",
          "bizTypNm": "공유수면점용",
          "bizTypEtc": "공유수면 점사용",
          "adrSido": "울산",
          "adrGugun": "동구",
          "adrDong": "방어동",
          "adrDtl": "1425번지 및 1381번지 공유수면",
          "bldDat": "2013-06-01",
          "endDat": "2014-05-31",
          "cst": "사업비(원)",
          "bizSiz": "1,262.25㎡ (직접 12.25㎡ / 간접 1,250㎡)",
          "bizAim": "선박 피해방지용 침추 설치 (현대미포조선 3안벽 전면)",
          "prcIst": "울산지방해양수산청",
          "etcDes": "비고",
          "dsSts": "조건부동의",
          "lat": 129.399,
...
| **필수 파라미터 포함** | 완료 | `ServiceKey`, `pageNo`, `numOfRows`, `ACP_YEAR`가 누락되지 않았는지 확인 |
...
| **참고 문서 확인** | 확인 필요 | 참고 문서 `(복구중)_1.해역이용_협의정보_서비스_활용_가이드.docx`를 추가로 확인하여 누락된 정보가 없는지 검토 |
