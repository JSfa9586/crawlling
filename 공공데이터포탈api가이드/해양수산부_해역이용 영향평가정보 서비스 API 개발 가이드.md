# 해양수산부_해역이용 영향평가정보 서비스 API 개발 가이드

## 목차
1. 개요
2. 인증 및 공통 사항
3. API 분류별 상세
    3.1. 해역이용 영향평가정보 목록 조회 (getOceansSeaareaImpactInfo1)
4. 에러 코드 및 트러블슈팅
5. 개발 체크리스트

## 1. 개요

본 문서는 **해양수산부_해역이용 영향평가정보 서비스** API를 활용하여 개발을 진행하는 개발자를 위한 가이드라인입니다. 이 API는 해역이용 영향평가 정보를 조회하는 기능을 제공합니다.

*   **API 서비스명:** 해양수산부_해역이용 영향평가정보 서비스
*   **설명:** 접수연도, 사업명을 이용하여 사업번호, 접수일, 결과명, 진행단계명, 사업명, 사업시행자, 주소, 사업 시작/종료일, 사업비, 사업규모, 사업목적, 처분기관, 초안 공람 시작/종료일, 협의 상태, 위도, 경도 등 상세정보를 조회하는 서비스입니다.
*   **API 유형:** REST
*   **데이터 포맷:** JSON, XML
*   **키워드:** 해역, 해역이용, 협의정보
...
해역이용 영향평가 정보를 목록 형태로 조회합니다.

*   **분류/카테고리:** 해역이용 영향평가정보 조회
*   **기능 설명:** 접수연도, 사업명 등의 검색 조건을 이용하여 해역이용 영향평가 정보를 조회합니다.
*   **요청 주소 (Endpoint URL):** `http://apis.data.go.kr/1192000/service/OceansSeaareaImpactService1/getOceansSeaareaImpactInfo1`
*   **서비스 URL:** `http://apis.data.go.kr/1192000/service/OceansSeaareaImpactService1`
...
| SEQUENCE | seq | 선택 | Integer | 10 | 2 | SEQUENCE |
| 사업번호 | bizNum | 선택 | String | 30 | 2121219001 | 사업번호 |
| 접수연도 | acpYear | 필수 | String | 4 | 2011 | 접수연도 |
| 접수일 | acpDat | 선택 | String | 30 | 2011-05-25 | 접수일 |
| 결과명 | rstNm | 선택 | String | 20 | 조건부동의 | 결과명 |
| 진행단계명 | stpNm | 선택 | String | 20 | 협의완료 | 진행단계명 |
| 사업명 | bizNam | 선택 | String | 30 | 안산 풍도.육도 공유수면내 골재채취 사업 해양환경영향조사서 | 사업명 |
| 사업시행자 | bizActNam | 선택 | String | 30 | ㈜장강산업개발 | 사업시행자 |
| 사업유형명 | bizTypNm | 선택 | String | 30 | 바다골재채취 | 사업유형명 |
| 사업유형 기타 | bizTypEtc | 선택 | String | 1000 | 사업유형 기타 |
| 주소시도 | adrSido | **필수** | String | 30 | 경기 | 주소시도 |
| 주소구군 | adrGugun | 선택 | String | 30 | 안산시 | 주소구군 |
| 주소읍면동 | adrDong | 선택 | String | 30 | 풍도동 | 주소읍면동 |
| 사업 소재지 상세주소 | adrDtl | 선택 | String | 100 | 동쪽 지선앞 공유수면 | 사업 소재지 상세주소 |
| 사업 시작일 | bldDat | 선택 | String | 30 | 2009-12-31 | 사업 시작일 |
| 사업 종료일 | endDat | 선택 | String | 30 | 2013-12-29 | 사업 종료일 |
| 사업비 | cst | 선택 | String | 12 | 사업비(원) |
| 사업규모 | bizSiz | 선택 | String | 1000 | 5200만㎥ | 사업규모 |
| 사업목적 | bizAim | 선택 | String | 1000 | 골재채취 | 사업목적 |
| 처분기관 | prcIst | 선택 | String | 30 | 평택지방해양수산청 | 처분기관 |
| 초안 공람 시작일 | drfBldDat | 선택 | String | 30 | 초안 공람 시작일 |
| 초안 공람 종료일 | drfEndDat | 선택 | String | 30 | 초안 공람 종료일 |
| 비고 | etcDes | 선택 | String | 1000 | 비고 |
| 협의 상태 | dsSts | 선택 | String | 30 | 협의완료 | 협의 상태 |
...
curl -X GET "http://apis.data.go.kr/1192000/service/OceansSeaareaImpactService1/getOceansSeaareaImpactInfo1?ServiceKey={YOUR_SERVICE_KEY_URL_ENCODED}&pageNo=1&numOfRows=10&ACP_YEAR=2013&resultType=json"
```
      ...
      "items": [
        {
          ...
          "rstNm": "조건부동의",
          "stpNm": "협의완료",
          "bizNam": "안산 풍도.육도 공유수면내 골재채취 사업 해양환경영향조사서",
          "bizActNam": "㈜장강산업개발",
          "bizTypNm": "바다골재채취",
          "bizTypEtc": "사업유형 기타",
          "adrSido": "경기",
...
| **참고 문서 확인** | 확인 필요 | 참고 문서 `(복구중)_4.해역이용_영향평가정보_서비스_활용_가이드.docx`를 추가로 확인하여 누락된 정보가 없는지 검토 |
