# 해양수산부_해역이용협의 평가대행자 정보 서비스 API 개발 가이드

## 목차
1. 개요
2. 인증 및 공통 사항
3. API 분류별 상세
    3.1. 해역이용 평가대행자 정보 조회 (GET /EvaluationAgentInfo)
4. 에러 코드 및 트러블슈팅
5. 개발 체크리스트

## 1. 개요

본 문서는 **해양수산부_해역이용협의 평가대행자 정보 서비스** API를 활용하여 개발을 진행하는 개발자를 위한 가이드라인입니다. 이 API는 해역이용협의 평가대행자의 관련 정보를 조회하는 기능을 제공합니다.

*   **API 서비스명:** 해양수산부_해역이용협의 평가대행자 정보 서비스
*   **설명:** 해역이용평가대행자 등록번호, 등록일, 업체명, 대표자명, 주소, 전화번호, 관할해양지방수산청명 등 평가대행자의 상세 정보를 제공합니다.
*   **API 유형:** REST
*   **데이터 포맷:** XML
*   **키워드:** 해역이용협의, 평가대행자, 해역이용협의 평가대행자, 현황파악, 등록정보
...
해역이용협의 평가대행자 정보를 목록 형태로 조회합니다.

*   **분류/카테고리:** 평가대행자 정보 조회
*   **기능 설명:** 해역이용 평가대행자의 상세 정보를 조회합니다.
*   **Base URL:** `http://apis.data.go.kr/1192000/EvaluationAgentInfoService`
*   **엔드포인트:** `/EvaluationAgentInfo`
*   **전체 요청 주소:** `http://apis.data.go.kr/1192000/EvaluationAgentInfoService/getEvaluationAgentInfo`
...
| 등록번호 | REG\_NUM | 선택 | String | 100 | 2017-01 | 해역이용평가대행자 등록번호 |
| 업체명 | AGENT\_NM | 선택 | String | 100 | 한국해양과학기술원 | 평가대행업체명 |
...
| 등록번호 | regNum | 선택 | String | 100 | 2017-01 | 해역이용평가대행자 등록번호 |
| 등록일 | regDat | 선택 | String | 10 | 2017-01-01 | 평가대행자 등록일 |
| 업체명 | agentNm | 선택 | String | 100 | 한국해양과학기술원 | 평가대행업체명 |
| 대표자명 | ceoNm | 선택 | String | 100 | 홍길동 | 대표자명 |
| 사무실주소 | offAddr | 선택 | String | 100 | 부산광역시 영도구 해양로 385 | 사무실 주소 |
| 평가부서주소 | evalAddr | 선택 | String | 100 | 부산광역시 영도구 해양로 385 | 평가부서 주소 |
| 사무실전화번호 | offTel | 선택 | String | 100 | 051-500-1234 | 사무실 전화번호 |
| 평가부서전화번호 | evalTel | 선택 | String | 100 | 051-500-1234 | 평가부서 전화번호 |
| 관할해양지방수산청명 | moffNm | 선택 | String | 100 | 부산지방해양수산청 | 관할 해양지방수산청명 |
...
```bash
# 요청 변수: ServiceKey (인증키), pageNo=1, numOfRows=10, AGENT_NM=한국해양과학기술원
# ServiceKey, pageNo, numOfRows는 필수 파라미터입니다.
# AGENT_NM은 URL 인코딩이 필요합니다.

curl -X GET "http://apis.data.go.kr/1192000/EvaluationAgentInfoService/getEvaluationAgentInfo?ServiceKey={YOUR_SERVICE_KEY_URL_ENCODED}&pageNo=1&numOfRows=10&AGENT_NM=%ED%95%9C%EA%B5%AD%ED%95%B4%EC%96%91%EA%B3%BC%ED%95%99%EA%B8%B0%EC%88%A0%EC%9B%90"
```
    ...
    <body>
        <items>
            <item>
                <num>1</num>
                <regNum>2017-01</num>
                <regDat>2017-01-01</regDat>
                <agentNm>한국해양과학기술원</agentNm>
                <ceoNm>홍길동</ceoNm>
                <offAddr>부산광역시 영도구 해양로 385</offAddr>
                <evalAddr>부산광역시 영도구 해양로 385</evalAddr>
                <offTel>051-500-1234</offTel>
                <evalTel>051-500-1234</evalTel>
                <moffNm>부산지방해양수산청</moffNm>
            </item>
            <!-- ... more items ... -->
        </items>
...
| **한글 인코딩 확인** | 완료 | `AGENT_NM`과 같은 한글 파라미터는 URL 인코딩을 통해 전달했는지 확인 |
