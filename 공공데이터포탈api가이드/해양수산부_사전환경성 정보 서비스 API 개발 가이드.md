# 해양수산부_사전환경성 정보 서비스 API 개발 가이드

## 목차
1. 개요
2. 인증 및 공통 사항
3. API 분류별 상세
    3.1. 사전환경성 정보 목록 조회 (getPreexmntEnvInfo2)
4. 에러 코드 및 트러블슈팅
5. 개발 체크리스트

## 1. 개요

본 문서는 **해양수산부_사전환경성 정보 서비스** API를 활용하여 개발을 진행하는 개발자를 위한 가이드라인입니다. 이 API는 사전환경성 검토 정보를 조회하는 기능을 제공합니다.

*   **API 서비스명:** 해양수산부_사전환경성 정보 서비스
*   **설명:** 접수연도, 사업명을 이용하여 사전환경성 고유번호, 사업번호, 접수일, 결과명, 진행단계명, 사업명, 사업시행자, 주소, 사업 시작/종료일, 사업비, 사업규모, 사업목적, 처분기관, 초안 공람 시작/종료일, 협의 상태, 위도, 경도 등 상세정보를 조회하는 서비스입니다.
*   **API 유형:** REST
*   **데이터 포맷:** JSON, XML
*   **키워드:** 해역, 해역이용, 협의정보
...
사전환경성 검토 정보를 목록 형태로 조회합니다.

*   **분류/카테고리:** 사전환경성 정보 조회
*   **기능 설명:** 접수연도, 사업명 등의 검색 조건을 이용하여 사전환경성 정보를 조회합니다.
*   **요청 주소 (Endpoint URL):** `http://apis.data.go.kr/1192000/service/PreexmntEnvService2/getPreexmntEnvInfo2`
*   **서비스 URL:** `http://apis.data.go.kr/1192000/service/PreexmntEnvService2`
...
| 사업명 | BIZ\_NAM | 선택 | String | 100 | 신안 흑산 생활체육공원 조성사업(초안) | 사업명 |
| 결과형식 | resultType | 선택 | String | 4 | XML/JSON | 결과형식 (값이 없으면 XML로 자동설정됨) |

#### 3.1.2. 응답 필드 상세 (Response Fields)

**주의:** 공공데이터포털 문서의 '출력결과(Response Element)' 테이블에는 `num`, `acpYear`, `bizNam` 세 가지 필드만 명시되어 있습니다. 그러나 '기능 설명'에는 더 많은 항목이 언급되어 있습니다. 실제 응답은 '기능 설명'에 언급된 모든 필드를 포함할 가능성이 높으나, 문서에 명시된 필드만 정확히 기재합니다.

| 항목명(국문) | 항목명(영문) | 필수/선택 | 타입 | 항목크기 | 샘플데이터 | 항목설명 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 일련번호 | num | 필수 | Integer | 10 | 1 | 일련번호 |
| 접수연도 | acpYear | 필수 | String | 4 | 2013 | 접수연도 |
| 사업명 | bizNam | 선택 | String | 100 | 신안 흑산 생활체육공원 조성사업(초안) | 사업명 |

**참고:** 기능 설명에 언급되었으나 응답 필드 테이블에 누락된 항목: `사전환경성 고유번호`, `SEQUENCE`, `사업번호`, `접수일`, `결과명`, `진행단계명`, `사업시행자`, `사업유형명`, `사업유형기타`, `주소시도`, `주소구군`, `주소읍면동`, `사업소재지 상세주소`, `사업시작일`, `사업종료일`, `사업비`, `사업규모`, `사업목적`, `처분기관`, `초안 공람 시작일`, `초안 공람 종료일`, `비고`, `협의상태`, `협의서 종류명`, `위도`, `경도`.
...
```bash
# 요청 변수: ServiceKey (인증키), pageNo=1, numOfRows=10, ACP_YEAR=2013
# BIZ_NAM은 선택 사항입니다.
# ServiceKey, pageNo, numOfRows, ACP_YEAR는 필수 파라미터입니다.

curl -X GET "http://apis.data.go.kr/1192000/service/PreexmntEnvService2/getPreexmntEnvInfo2?ServiceKey={YOUR_SERVICE_KEY_URL_ENCODED}&pageNo=1&numOfRows=10&ACP_YEAR=2013&resultType=json"
```
      ...
      "items": [
        {
          "num": 1,
          "acpYear": "2013",
          "bizNam": "신안 흑산 생활체육공원 조성사업(초안)"
          // 실제 응답에는 기능 설명에 언급된 다른 필드들이 포함될 수 있습니다.
        }
      ],
      "numOfRows": 10,
      "pageNo": 1,
...
| **응답 필드 누락 가능성 인지** | 완료 | 문서에 명시된 응답 필드가 기능 설명과 불일치함. 실제 응답을 확인하며 개발 진행 필요. |
| **한글 인코딩 확인** | 완료 | `BIZ_NAM`과 같은 한글 파라미터는 URL 인코딩을 통해 전달했는지 확인 |
| **참고 문서 확인** | 확인 필요 | 참고 문서 `(복구중)_2.사전환경성_정보_서비스_활용_가이드.docx`를 추가로 확인하여 누락된 정보가 없는지 검토 |
