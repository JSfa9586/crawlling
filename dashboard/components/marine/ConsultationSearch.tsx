...
export default function ConsultationSearch() {
    const [allItems, setAllItems] = useState<ConsultationItem[]>([]);
    ...
    const handleSearch = async () => {
        ...
        try {
            const allResults: ConsultationItem[] = [];
            const years = [];
            ...
            for (let i = 0; i < years.length; i++) {
                ...
                const res = await fetch(`/api/public-data/consultation?pageNo=1&numOfRows=3000&acpYear=${year}`);
                ...
                const fetchedItems = data.getOceansUseInfo?.item || data.response?.body?.items?.item || [];
            ...
            {allItems.length > 0 && (
                <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                    <div className="flex gap-4 flex-wrap items-end">
                        ...
                        {/* 진행상태 필터 */}
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">협의상태</label>
                            <select
                ...
                <table className="min-w-full border-collapse border border-gray-200 table-fixed">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="border p-2 text-left text-gray-700 w-[100px]">협의상태</th>
                    ...
                    <tbody>
                        ...
                        {paginatedItems.map((item, idx) => (
                            <tr key={idx} className="hover:bg-gray-50">
                                ...
                                <td className="border p-2 text-gray-800 text-sm whitespace-nowrap">
                                    <span className={`px-2 py-1 rounded text-xs ${item.dsSts === '조건부동의' ? 'bg-green-100 text-green-800' :
                                        item.dsSts === '반려' ? 'bg-red-100 text-red-800' :
