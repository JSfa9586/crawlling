        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-2 text-gray-600">자료를 게시물(폴더) 단위로 관리합니다.</p>
                </div>
                <button
            ...
            {isCreating && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        <h3 className="text-lg font-bold mb-4">새 게시물 만들기</h3>
            ...
            <div className="bg-white shadow overflow-hidden rounded-lg">
                {isLoading ? (
                    ...
                    <div className="p-8 text-center text-gray-500">등록된 게시물이 없습니다.</div>
                ) : (
                    <ul className="divide-y divide-gray-200">
                        {items.map((item) => (
                            <li key={item.id} className="hover:bg-gray-50 transition-colors">
                                <Link href={`/dashboard/test/reference/${item.id}?name=${encodeURIComponent(item.name)}`} className="block p-4">
