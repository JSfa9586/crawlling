...
export default function ReferencePostPage() {
    const params = useParams();
    const searchParams = useSearchParams();
    const folderId = params.id as string;
    const folderName = searchParams.get('name') || '게시물 상세';
    ...
    const fetchFiles = useCallback(async () => {
        try {
            ...
            if (result.success) {
                // 파일만 필터링 (폴더 제외)
                const fileList = (result.data as DriveFile[]).filter(item =>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="mb-6">
                <div className="flex items-center text-sm text-gray-500 mb-2">
                    ...
                    <Link href="/dashboard/test/reference" className="hover:text-blue-600">테스트 자료실</Link>
                    <svg className="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
            ...
            <div className="space-y-8">
                {/* 업로드 섹션 */}
                <section>
                    <h2 className="text-lg font-semibold text-gray-900 mb-4">파일 추가</h2>
                    <FileUploader
                        folderId={folderId}
                ...
                {/* 파일 목록 섹션 */}
                <section>
                    <h2 className="text-lg font-semibold text-gray-900 mb-4">첨부 파일 ({files.length})</h2>
                    {isLoading ? (
